...
##############################################################################
#Cleaning out all variable from memory first
rm(list=ls())
gc()
##############################################################################
makeVector <- function(x = numeric()) {
...
#cacheSolve calculates the inverse of what is returned from makeCacheMatrix
#if the inverse already exists, then cached value will be used in order to
#reduce the computational overhead. Only if the reverse does not exist already will 
#the reverse be calculated. 
...
##############################################################################
# now attempt to inverse the of the matrix. but if it is already in cache, directly return from cache
...
## > cacheSolve(m)
## getting cached data.  <---- NOTICE THAT IT RETURNED CACHED RESULT
##           [,1]      [,2]
## [1,] 1.0666667 0.2666667
## [2,] 0.2666667 1.0666667
